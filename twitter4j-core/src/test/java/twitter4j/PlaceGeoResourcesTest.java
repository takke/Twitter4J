...
class PlaceGeoResourcesTest extends TwitterTestBase {
    ...
    void testGeoMethods() throws Exception {
        ...
        places = twitter1.v1Resources().reverseGeoCode(query);
        ...
        places = twitter1.v1Resources().searchPlaces(query);
        assertNotNull(TwitterObjectFactory.getRawJSON(places));
        assertEquals(places.get(0), TwitterObjectFactory.createPlace(TwitterObjectFactory.getRawJSON(places.get(0))));
        assertTrue(places.size() > 0);
        places = twitter1.v1Resources().getSimilarPlaces(new GeoLocation(37.78215, -122.40060), "SoMa", null, null);
        assertNotNull(TwitterObjectFactory.getRawJSON(places));
        assertEquals(places.get(0), TwitterObjectFactory.createPlace(TwitterObjectFactory.getRawJSON(places.get(0))));
        assertTrue(places.size() > 0);

        try {
            Place place = twitter1.v1Resources().getGeoDetails("5a110d312052166f");
        ...
        Status status = twitter1.v1Resources().updateStatus(new StatusUpdate(new java.util.Date() + " status with place").
    ...
    void testGeoLocation() throws Exception {
        ...
        Status withgeo = twitter1.v1Resources().updateStatus(new StatusUpdate(new java.util.Date().toString() + ": updating geo location").location(new GeoLocation(LATITUDE, LONGITUDE)));
        ...
        assertFalse(twitter2.v1Resources().verifyCredentials().isGeoEnabled());
    }
