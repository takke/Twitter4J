...
class SuggestedUsersResourcesTest extends TwitterTestBase {
    ...
    void testSuggestion() throws Exception {
        ResponseList<Category> categories = twitter1.v1Resources().getSuggestedUserCategories();
        ...
        ResponseList<User> users = twitter1.v1Resources().getUserSuggestions(categories.get(0).getSlug());
        ...
        users = twitter1.v1Resources().getMemberSuggestions(categories.get(0).getSlug());
        assertTrue(users.size() > 0);
