...
import twitter4a.auth.AuthorizationFactory;
import twitter4a.auth.OAuthAuthorization;
...
public class TwitterFactory implements java.io.Serializable {
    ...
    static {
        ...
        if (gaeDetected) {
            final String APP_ENGINE_TWITTER_IMPL = "twitter4a.AppEngineTwitterImpl";
            try {
        ...
        if (className == null) {
            className = "twitter4a.TwitterImpl";
        }
     ...
     * Unlike {@link Twitter#setOAuthAccessToken(twitter4a.auth.AccessToken)}, this factory method potentially returns a cached instance.
     *
